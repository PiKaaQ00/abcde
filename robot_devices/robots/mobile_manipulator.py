...
class MobileManipulator:
    ...
    def __init__(self, config: LeKiwiRobotConfig):
        ...
        # This will be overwritten as soon as we receive the first message
        self.last_remote_arm_state = torch.zeros(6, dtype=torch.float32)

        # For human-takeover teleop: offsets between follower and master joints
        # teleop_joint_offsets[name] is a tensor of length 6:
        #   offset = follower_joint - master_joint at takeover time.
        self.teleop_joint_offsets: dict[str, torch.Tensor] = {}
    ...
    def motor_features(self) -> dict:
        ...
        observations = ["x_mm", "y_mm", "theta"]  # h_mm is the lift axis in mm/s
    ...
    def align_master_to_follower_for_teleop(self):
        """
        To be called exactly once when switching from POLICY control to HUMAN takeover.

        It reads the current follower (remote) arm joint positions and the current
        master (leader) arm positions, and computes a per-arm offset:

            offset = follower_joint - master_joint

        Later, teleop_step will send:

            cmd = master_joint + offset

        so that at the takeover instant, follower_joints stay continuous,
        and you only do *relative* motion around the current follower pose.
        """
        if not self.is_connected:
            raise RobotDeviceNotConnectedError("Not connected. Run `connect()` first.")

        # 1) Get follower + base state
        obs = self.capture_observation()
        state: torch.Tensor = obs["observation.state"]
        ...
        joints_per_arm = len(follower_arm_names)

        follower_arm_keys = list(self.follower_arms.keys())
        num_follower_arms = len(follower_arm_keys)
        total_arm_joints = joints_per_arm * num_follower_arms

        follower_arm_state = state[:total_arm_joints]

        # 2) Read master joints
        master_positions: dict[str, torch.Tensor] = {}
        for name, arm in self.leader_arms.items():
            pos = arm.read("Present_Position")  # numpy
            master_positions[name] = torch.from_numpy(pos).float()

        # 3) Compute offsets per arm
        self.teleop_joint_offsets = {}
        idx = 0
        for arm_name in follower_arm_keys:
            if arm_name not in master_positions:
                idx += joints_per_arm
                continue

            follower_slice = follower_arm_state[idx : idx + joints_per_arm]
            master_slice = master_positions[arm_name]

            # If master arm has more joints (e.g. includes gripper differently),
            # take the first 'joints_per_arm' entries.
            if master_slice.shape[0] != joints_per_arm:
                master_slice = master_slice[:joints_per_arm]

            self.teleop_joint_offsets[arm_name] = follower_slice - master_slice
            idx += joints_per_arm

        print("[TAKEOVER] teleop joint offsets updated for HUMAN control")
    ...
    def teleop_step(
        ...
        # Here we optionally apply an offset so that:
        #   cmd = master_joint + offset
        # When no offset is set (normal teleop), this is just direct mapping.
        arm_positions: dict[str, list[float]] = {}
        for name, arm in self.leader_arms.items():
            pos = arm.read("Present_Position")
            pos_tensor = torch.from_numpy(pos).float()

            if name in self.teleop_joint_offsets:
                pos_tensor = pos_tensor + self.teleop_joint_offsets[name]

            arm_positions[name] = pos_tensor.tolist()
        ...
        # ---- 数据记录部分：统一使用“从臂 + 底盘真实状态” ----
        # 这里不再用主臂角度来构造 action，而是直接用 follower 状态，
        # 确保数据集中 action 永远代表从臂的行为。
        obs_dict = self.capture_observation()
        state_tensor: torch.Tensor = obs_dict["observation.state"]
        action_dict = {"action": state_tensor.clone()}
    ...
    def send_action(self, action: torch.Tensor) -> torch.Tensor:
        ...
        num_per_arm = 6  # e.g. 6 joints + 1 gripper
